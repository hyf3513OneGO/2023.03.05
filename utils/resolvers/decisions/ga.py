...
class MyProblem(ea.Problem):  # 继承Problem父类
    def __init__(self, taskTimeMats, taskDataMats, decisionsMats, resourceMats, carInfos, MECInfos, B):
        ...
        varTypes = [1 for _ in range(lenV)]  # 初始化varTypes（决策变量的类型，0：实数；1：整数）
        lb = [0 for _ in range(lenV)]  # 决策变量下界
        ub = [1 for _ in range(lenV)]  # 决策变量上界
...
def gaResolve_class_decision(taskTimeMats, tasksDataMats, decisionMat, ResourceMats, CarInfos, MECInfo, B):
    # 实例化问题对象
