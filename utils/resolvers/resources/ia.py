...
class MyProblem:
    ...
    def aimFunc(self, Chrom):
        ...
        if taskEnergyTotal / taskEnergyOriginTotal > 2:
            return 1000000
        return -QOETotal
    ...
    def solve(self):
        ...
        # lbin = [1 for _ in range(lenV)]  # 决策变量下边界（0表示不包含该变量的下边界，1表示包含）
        # ubin = [1 for _ in range(lenV)]  # 决策变量上边界（0表示不包含该变量的上边界，1表示包含）
        ia = IA_TSP(func=self.aimFunc, n_dim=lenV, size_pop=40, max_iter=max_iter)
        ia.run()
        decisionMatResult, _ = geatVars2mat(ia.best_x, self.taskTimeMats)
...
def iaResolve_class_resource(taskTimeMats, tasksDataMats, decisionMat, ResourceMats, CarInfos,
                             MECInfo, B):
